...
function parseAvisList(raw) {
  const src = Array.isArray(raw)
    ? raw
    : [0, 1, 2].map((i) => ({
        salle: raw?.[`avis_salle_${i}`] || raw?.[`avis${i}_salle`],
        proof: raw?.[`avis_proof_${i}`] || raw?.[`avis${i}_proof`],
      }));
  const seen = new Set();
  const avis = [];
  for (const item of src.slice(0, 3)) {
    ...
    if (seen.has(salleId)) {
      return { ok: false, error: { field: 'avis', message: 'Chaque avis doit viser une salle différente' }, avis: [] };
    }
...
/** 1 ticket de base, +1 par avis Google (screen) — max x4. */
export function ticketCount({ avis = [] } = {}) {
  return 1 + Math.min(3, Array.isArray(avis) ? avis.length : 0);
}
