...
import { invitationMessage } from './messages.js';
...
function parseAvisList(raw) {
  ...
  if (!avis.length) {
    return { ok: false, error: { field: 'avis', message: 'Avis Google obligatoire (screen)' }, avis: [] };
  }
  return { ok: true, avis };
}

function friendTouched(f) {
  return Boolean(f?.prenom || f?.nom || f?.telephone || f?.email);
}

function friendComplete(f) {
  return Boolean(f?.prenom && f?.nom && f?.telephone);
}

export function completeFriends(friends = []) {
  return (Array.isArray(friends) ? friends : []).filter(friendComplete);
}

/** Infos + avis Google = 1 ticket. +1 ticket si 2 ami(e)s complets. Max 2. */
export function ticketCount({ avis = [], friends = [] } = {}) {
  if (!Array.isArray(avis) || avis.length === 0) return 0;
  return 1 + (completeFriends(friends).length >= 2 ? 1 : 0);
}
...
export function parseEntry(body) {
  ...
  const touched = friendsRaw.filter(friendTouched);
  let friends = [];
  if (touched.length === 0) {
    friends = [];
  } else if (friendsRaw.filter(friendComplete).length < 2) {
    errors.push({
      field: 'friends',
      message: 'Pour le ticket bonus, renseigne tes 2 ami(e)s (prénom, nom, téléphone) ou laisse les deux vides',
    });
    friends = friendsRaw;
    friendsRaw.forEach((f, i) => {
      const n = i + 1;
      if (friendTouched(f) || touched.length > 0) {
  ...
  if (friends.length >= 2 && !truthy(body.consent_friends)) {
    errors.push({ field: 'consent_friends', message: 'Confirmation d’information des ami(e)s requise' });
  }
  if (!truthy(body.consent_privacy)) {
    errors.push({ field: 'consent_privacy', message: 'Acceptation de la politique de confidentialité requise' });
  }

  const pKey = phoneKey(telephoneRaw);
  const fKeys = friends.map((f) => phoneKey(f.telephone)).filter(Boolean);
  if (friends.length >= 2) {
...
export async function enterContest(body, { publicUrl, dryRun = false } = {}) {
  ...
  if (sendConfirm && participant.wa_status !== 'skipped') {
    await upsertContact({ ...participant, wa_status: 'skipped', wa_error: 'sms_disabled' });
  }
  ...
  for (const job of jobs) {
    if (job.kind === 'confirmation') {
      ...
      await updateQueue(job.id, {
        ...
        last_error: 'sms_disabled',
      });
      const contact = await getContactById(job.contact_id);
      if (contact) {
        await upsertContact({ ...contact, wa_status: 'skipped', wa_error: 'sms_disabled' });
      }
      continue;
    }
    try {
...
export function kpis({ contacts, invites, events, queue, since } = {}) {
  const ev = sinceFilter(events, since);
  const ct = sinceFilter(contacts, since);
  const inv = sinceFilter(invites, since);
  const qu = sinceFilter(queue, since);
  const visitors = ev.filter((e) => e.type === 'page_vue').length;
  const formStarted = ev.filter((e) => e.type === 'form_start').length;
  const formSubmit = ev.filter((e) => e.type === 'form_submit').length;
  const inscrits = ct.filter((c) => c.status === 'inscrit' || c.status === 'inscription_finalisee').length;
  const withAvis = ct.filter((c) => Array.isArray(c.avis) && c.avis.length > 0).length;
  const ticketsTotal = ct.reduce((sum, c) => {
    const avisN = Array.isArray(c.avis) ? c.avis.length : 0;
    return sum + Number(c.tickets || 1 + avisN);
  }, 0);
  const invitesCount = ct.filter(
    (c) => c.invited_by_id || c.role === 'invite' || c.status === 'invite'
  ).length;
  const finalises = ct.filter((c) => c.status === 'inscription_finalisee').length;
  const waSent = qu.filter((q) => q.status === 'sent').length;
  const waErrors = ct.filter((c) => c.wa_status === 'error').length + ev.filter((e) => e.type === 'wa_error').length;
  const duplicates = ev.filter((e) => e.type === 'duplicate').length + ct.filter((c) => c.status === 'doublon').length;
  const bySource = {};
  for (const c of ct) {
  ...
  for (const row of inv) {
    if (!row.inviter_id || !row.invitee_id) continue;
    generatedBy[row.inviter_id] = (generatedBy[row.inviter_id] || 0) + 1;
    countedInvitees.add(row.invitee_id);
  }
  for (const c of ct) {
