...
const META_FILE = path.join(__dirname, '..', 'data', 'concours-meta.json');
const STATS_RESET_KEY = 'concours_stats_reset_at';
...
/** PostgREST renvoie ~1000 lignes max sans pagination — obligatoire pour l’admin. */
async function listAllSupabase(pathname, queryParts = []) {
  const rows = [];
  let offset = 0;
  for (;;) {
    const params = [...queryParts, `offset=${offset}`, `limit=${PAGE_SIZE}`];
    const res = await fetchJson(supabaseUrl(pathname, `?${params.join('&')}`), {
      headers: {
        ...supabaseHeaders(),
    ...
    if (!res.ok) {
      throw new Error(`Supabase list ${pathname}: ${res.status} ${String(res.text).slice(0, 180)}`);
    }
...
export async function addInvite({ inviter_id, invitee_id }) {
  ...
  if (kind === 'supabase') {
    ...
    if (inserted.ok) return row;
    // Doublon (même parrain → même ami) : ne pas bloquer l’inscription.
    if (inserted.status === 409) {
      const existing = await fetchJson(
        supabaseUrl(
          'concours_invites',
          `?inviter_id=eq.${encodeURIComponent(inviter_id)}&invitee_id=eq.${encodeURIComponent(invitee_id)}&select=*&limit=1`
        ),
        { headers: supabaseHeaders() }
      );
    ...
    throw new Error(`Supabase invite: ${inserted.status} ${String(inserted.text).slice(0, 180)}`);
  }
  const data = await load();
...
export async function listContacts({ status, source, from, to } = {}) {
  ...
  if (kind === 'supabase') {
    ...
    rows = await listAllSupabase('concours_contacts', params);
  } else {
    const data = await load();
    rows = [...data.contacts];
...
export async function listInvites() {
  const kind = backend();
  if (kind === 'supabase') {
    return listAllSupabase('concours_invites', ['select=*', 'order=created_at.desc']);
  }
  const data = await load();
  return data.invites;
}
...
export async function listEvents() {
  ...
  if (kind === 'supabase') {
    try {
      return await listAllSupabase('concours_events', ['select=*', 'order=created_at.desc']);
    } catch {
      return [];
    }
  }
...
export async function getStatsResetAt() {
  const kind = backend();
  if (kind === 'supabase') {
    const res = await fetchJson(
      supabaseUrl('boxplus_store_config', `?key=eq.${encodeURIComponent(STATS_RESET_KEY)}&select=payload`),
      { headers: supabaseHeaders() }
    );
    if (!res.ok) return null;
    const row = Array.isArray(res.data) ? res.data[0] : null;
    const at = row?.payload?.stats_reset_at;
    return at ? String(at) : null;
  }
  const meta = await readMetaFile();
  return meta.stats_reset_at ? String(meta.stats_reset_at) : null;
}

async function setStatsResetAt(iso) {
  const kind = backend();
  if (kind === 'supabase') {
    const res = await fetchJson(supabaseUrl('boxplus_store_config'), {
      method: 'POST',
      headers: supabaseHeaders({ Prefer: 'resolution=merge-duplicates,return=minimal' }),
      body: JSON.stringify({
        key: STATS_RESET_KEY,
        payload: { stats_reset_at: iso },
        updated_at: iso,
      }),
    });
    if (!res.ok) {
      throw new Error(`Supabase meta: ${res.status} ${String(res.text).slice(0, 180)}`);
    }
    return iso;
  }
  const meta = await readMetaFile();
  meta.stats_reset_at = iso;
...
export async function clearAllEvents() {
  const kind = backend();
  if (kind === 'supabase') {
    const res = await fetchJson(supabaseUrl('concours_events', '?id=not.is.null'), {
      method: 'DELETE',
      headers: supabaseHeaders({ Prefer: 'return=minimal' }),
    });
    if (!res.ok) {
      throw new Error(`Supabase clear events: ${res.status} ${String(res.text).slice(0, 180)}`);
    }
    return;
  }
  const data = await load();
...
/** Remet les compteurs admin à zéro sans supprimer les contacts. */
export async function resetContestStats() {
...
export async function listQueueAll() {
  ...
  if (kind === 'supabase') {
    try {
      return await listAllSupabase('concours_wa_queue', ['select=*', 'order=created_at.desc']);
    } catch {
      return [];
    }
  }
