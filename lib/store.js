  ...
  if (kind === 'supabase') {
    ...
    if (existing) {
      ...
      if (!patched.ok && row.avis) {
        ...
        if (retry.ok) {
          const next = Array.isArray(retry.data) ? retry.data[0] : retry.data;
          return next || rest;
        }
      }
...
export async function syncPortetClient(contact) {
  ...
  const salle = salleLabel(contact.salle);
  const core = {
    prenom: contact.prenom || null,
    nom: contact.nom || null,
    ...
    const patch = await fetchJson(
      supabaseUrl('portet_clients', `?id=eq.${encodeURIComponent(existingId)}`),
      {
        ...
        body: JSON.stringify({
          ...
          offre: offerTag(contact),
          updated_at: now,
        }),
      }
    );
    if (patch.ok) return { ok: true, updated: true, id: existingId };
    const simple = await fetchJson(
      supabaseUrl('portet_clients', `?id=eq.${encodeURIComponent(existingId)}`),
      {
        method: 'PATCH',
        headers: supabaseHeaders(),
        body: JSON.stringify({ ...core, updated_at: now }),
      }
    );
    return { ok: simple.ok, updated: true, id: existingId };
  }

  const attempts = [
    { ...core, source: 'concours', offre: offerTag(contact), ville: contact.ville || null, created_at: now, updated_at: now },
  ...
  for (const row of attempts) {
    ...
    if (res.ok) return { ok: true, created: true, id: res.data?.[0]?.id || null };
  }
